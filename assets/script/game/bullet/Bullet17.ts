...
@ccclass('Bullet17')
export class Bullet17 extends BaseBullet {
        ...
        if (this.m_status == 1) {
            ...
            if (this.m_target) {
                ...
                if (this.m_lastDisX != 0 || this.m_lastDisY != 0) {
                    const distance = Math.sqrt(this.m_lastDisX * this.m_lastDisX + this.m_lastDisY * this.m_lastDisY);
                    const x = myWorldPosX + (this.m_lastDisX > 0 ? 1 : -1) * this.m_speed / 5 * deltaTime * (Math.abs(this.m_lastDisX) / distance);
                    ...
                    if (this.m_targetParent) {
                        ...
                        if (myWorldPosY < (targetWorldPosY + height / 2)) {
                            y = (targetWorldPosY + height / 2);
                            this.m_toBottom = true;
                        }
                        if (this.m_toBottom) {
                            y = (targetWorldPosY + height / 2);
                        }
    ...
    StartMove() {
        ...
        if (scaleTo) {
            scaleTo.setFrom(0);
            scaleTo.setTo(1);
            scaleTo.scaleTo(1 / smc.account.AccountModel.getGameRate(), () => { });
        }
    }
