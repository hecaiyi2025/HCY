...
@ccclass('Bullet15')
export class Bullet15 extends BaseBullet {
    ...
    StartMove() {
        ...
        this.m_backId = setTimeout(() => {
            if (this.m_moving == false) {
                const moveTo = this.node.addComponent(MoveTo);
                if (moveTo) {
                    moveTo.setFrom(this.node);
                    moveTo.setTo(this.m_from);
                    moveTo.moveTo(0.5 / smc.account.AccountModel.getGameRate(), () => { });
                }
                const scaleTo = this.node.addComponent(ScaleTo);
                if (scaleTo) {
                    scaleTo.setFrom(1);
                    scaleTo.setTo(0);
                    scaleTo.scaleTo(0.5 / smc.account.AccountModel.getGameRate(), () => { });
                }
            }
        }, (this.m_maxLive - 0.5 / smc.account.AccountModel.getGameRate()) * 1000);

        if (this.m_skeletons.length > 0) {
            this.m_skeletons[0].timeScale = 1.65 * smc.account.AccountModel.getGameRate();
        }
    }
